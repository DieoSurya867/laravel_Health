...
@section('content')
    <form action="{{ route('user.update', $artikel->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                          ...
                          @enderror"
                                ...
                                <option value="admin" {{ $user->id == $user->role ? 'selected' : '' }}>
                                    admin
                                </option>
                                <option value="editor" {{ $user->id == $user->role ? 'selected' : '' }}>
                                    editor
                                </option>
                                <option value="user" {{ $user->id == $user->role ? 'selected' : '' }}>
                                    user
                                </option>
